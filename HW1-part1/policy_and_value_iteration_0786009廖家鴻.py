...
def value_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
    ...
    ##### FINISH TODOS HERE #####    
    R_s_a_ns, P_s_a_ns = get_rewards_and_transitions_from_env(env)

    V = np.zeros(num_spaces)    
    for i in range(max_iterations):
        Q_s_a = np.einsum('ijk,ijk -> ij', P_s_a_ns, R_s_a_ns + gamma * V)
        
        best_values = np.max(Q_s_a,axis=1)
        v_itr = best_values
        delta = np.max(np.abs(v_itr - V))
        V = v_itr
        if delta < eps:
            break
    
    Q_s_a = np.einsum('ijk,ijk -> ij', P_s_a_ns, R_s_a_ns + gamma * V)
    best_action = np.argmax(Q_s_a,axis=1)
    policy = best_action
...
def policy_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
    ...
    for i in range(max_iterations): 
        ...
        R_s_ns = np.zeros([num_spaces, num_spaces])
        P_s_ns = np.zeros([num_spaces, num_spaces])
        
        while True:
            # delta = 0
            for s in range(num_spaces):
                R_s_ns[s] = R_s_a_ns[s][policy[s]][:]
                P_s_ns[s] = P_s_a_ns[s][policy[s]][:]
            v_itr = np.sum(P_s_ns*R_s_ns, axis=1) + gamma * P_s_ns.dot(V)
            delta = np.max(np.abs(v_itr - V))
            V = v_itr
            if delta < eps:
                break
        ...
        # R_s_a = np.sum(R_s_a_ns * P_s_a_ns, axis=2)
        # Q_s_a = R_s_a + gamma * P_s_a_ns.dot(V)
        Q_s_a = np.einsum('ijk,ijk -> ij', P_s_a_ns, R_s_a_ns + gamma * V)
        best_action = np.argmax(Q_s_a,axis=1)
        policy = best_action
